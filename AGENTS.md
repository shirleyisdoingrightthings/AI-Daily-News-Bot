...
| `~/Desktop/bots/shared/bot_utils.py` | 共享工具库（各 bot 共用）：基础工具 + 抓正文 + 跨天去重 + 相关性闸门 + 分页页码 + 代理自愈 + 零产监测，完整清单见文件头 | 偶尔 |
| `~/Desktop/bots/shared/scheduled-tasks/` | 10:00 定时任务 prompt 的版本化备份（原件在 `~/.claude/scheduled-tasks/`，跨各 bot，故放共享层而非本仓库）。改完 routine 记得同步回去 | 偶尔 |
